...
public class FailedSecurityOpsEventLog {

    private static final String TAG = "SecurityEventLog";
    private static Map<String, EventInfo> eventMap = new HashMap<>();

    // OBIS del log de "Failed Security Operation Event Logs"
    private static final String OBIS_FAILED_SECURITY = "0.0.99.98.12.255";

    // Grupo usado en events_table.json para los logs de seguridad (correctos y fallidos)
    private static final int GROUP_SECURITY = 7;

    public static List<EventFila> readFailedSecurityEventLog(Context context, GXDLMSReader reader, String from, String to) throws Exception {
        List<EventFila> result = new ArrayList<>();

        GXDLMSProfileGeneric securityLog = new GXDLMSProfileGeneric(OBIS_FAILED_SECURITY);

        AppLogger.i(TAG, "Leyendo estructura de Failed Security Operation Event Log...");
        ...
        if (securityLog.getCaptureObjects() == null || securityLog.getCaptureObjects().isEmpty()) {
            ...
            gurux.dlms.objects.GXDLMSData eventCode = new gurux.dlms.objects.GXDLMSData("0.0.96.11.12.255");
        ...
        AppLogger.i(TAG, "Solicitando eventos de operaciones de seguridad fallidas...");
        ...
        if (rows != null && rows.length > 0) {
            ...
            for (Object row : rows) {
                ...
                EventDescription info = getEventDescription(context, id, GROUP_SECURITY);
            ...
            AppLogger.i(TAG, "No se encontraron eventos de operaciones de seguridad fallidas.");
        }
        return result;
    }
