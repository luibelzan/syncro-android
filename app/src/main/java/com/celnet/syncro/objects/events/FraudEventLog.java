...
public class FraudEventLog {
    ...
    private static final String TAG = "FraudEventLog";
    private static Map<String, EventInfo> eventMap = new HashMap<>();

    public static List<EventFila> readFraudEventLog(Context context, GXDLMSReader reader, String from, String to) {
        ...
        try {
            ...
            AppLogger.i(TAG, "Leyendo estructura de Fraud Event Log...");
            reader.read(fraudLog, 3);

            if (fraudLog.getCaptureObjects() == null || fraudLog.getCaptureObjects().isEmpty()) {
                ...
                fraudLog.getCaptureObjects().add(new java.util.AbstractMap.SimpleEntry<>(clock, capClock));
                fraudLog.getCaptureObjects().add(new java.util.AbstractMap.SimpleEntry<>(eventCode, capEvent));
            }
            ...
            AppLogger.i(TAG, "Solicitando eventos de fraude...");
            ...
            if (rows != null && rows.length > 0) {
                ...
                for (Object row : rows) {
                    ...
                    EventDescription info = getEventDescription(context, id, 4);
                ...
                AppLogger.i(TAG, "No se encontraron eventos de fraude en el periodo solicitado.");
            }
        } catch (Exception e) {
            AppLogger.e(TAG, "Error en Fraud Log: " + e.getMessage());
        }
        return result;
    }
