...
public class CorrectSecurityOpsEventLog {

    private static final String TAG = "SecurityEventLog";
    private static Map<String, EventInfo> eventMap = new HashMap<>();

    // OBIS del log de "Correct Security Operation Event Logs"
    private static final String OBIS_CORRECT_SECURITY = "0.0.99.98.11.255";

    // ⚠️ TODO: confirmar el grupo real usado en events_table.json para este log
    // (el mismo criterio que "6" para Common Event Log en CommonEventLog).
    private static final int GROUP_CORRECT_SECURITY = 7;

    public static List<EventFila> readCorrectSecurityEventLog(Context context, GXDLMSReader reader, String from, String to) throws Exception {
        List<EventFila> result = new ArrayList<>();

        GXDLMSProfileGeneric securityLog = new GXDLMSProfileGeneric(OBIS_CORRECT_SECURITY);

        AppLogger.i(TAG, "Leyendo estructura de Correct Security Operation Event Log...");
        reader.read(securityLog, 3);

        if (securityLog.getCaptureObjects() == null || securityLog.getCaptureObjects().isEmpty()) {
            ...
            gurux.dlms.objects.GXDLMSData eventCode = new gurux.dlms.objects.GXDLMSData("0.0.96.11.11.255");
            ...
            securityLog.getCaptureObjects().add(new java.util.AbstractMap.SimpleEntry<>(clock, capClock));
            securityLog.getCaptureObjects().add(new java.util.AbstractMap.SimpleEntry<>(eventCode, capEvent));
        }
        ...
        AppLogger.i(TAG, "Solicitando eventos de operaciones de seguridad correctas...");
        Object[] rows = reader.readRowsByRange(securityLog, start, end);
        ...
        if (rows != null && rows.length > 0) {
            ...
            for (Object row : rows) {
                ...
                EventDescription info = getEventDescription(context, id, GROUP_CORRECT_SECURITY);
            ...
            AppLogger.i(TAG, "No se encontraron eventos de operaciones de seguridad correctas.");
        }
        return result;
    }
