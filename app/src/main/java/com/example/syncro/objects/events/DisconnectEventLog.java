package com.example.syncro.objects.events;

import com.example.syncro.client.GXDLMSReader;

import java.time.LocalDate;
import java.util.ArrayList;
import java.util.HashSet;
import java.util.List;
import java.util.Set;

import gurux.dlms.GXDateTime;
import gurux.dlms.enums.DateTimeSkips;
import gurux.dlms.objects.GXDLMSProfileGeneric;

public class DisconnectEventLog {

    public static List<Object[]> readDisconnectEventLog(GXDLMSReader reader, LocalDate from, LocalDate to) {
        List<Object[]> result = new ArrayList<>();
        try {
            // OBIS para Disconnect Event Log: 0.0.99.98.2.255
            String obisDisconnect = "0.0.99.98.2.255";
            GXDLMSProfileGeneric disconnectLog = new GXDLMSProfileGeneric(obisDisconnect);

            System.out.println("Leyendo estructura de Disconnect Event Log...");
            reader.read(disconnectLog, 3); // Lee los objetos capturados (columnas)

            // Configurar rango de fechas
            GXDateTime start = new GXDateTime(from.getYear(), from.getMonthValue(), from.getDayOfMonth(), 0, 0, 0, 0);
            GXDateTime end = new GXDateTime(to.getYear(), to.getMonthValue(), to.getDayOfMonth(), 23, 59, 59, 0);

            Set<DateTimeSkips> skips = new HashSet<>();
            skips.add(DateTimeSkips.DEVIATION); // Evita el error de zona horaria (FF C4)
            skips.add(DateTimeSkips.STATUS); // Evita el error de byte de estado

            // PASAR EL SET DIRECTAMENTE
            start.setSkip(skips);
            end.setSkip(skips);

            System.out.println("Solicitando eventos de desconexión...");
            Object[] rows = reader.readRowsByRange(disconnectLog, start, end);

            if (rows != null && rows.length > 0) {
                for (Object row : rows) {
                    Object[] fila = (Object[]) row;
                    result.add(fila);
                    System.out.println("DESCONEXIÓN: Fecha=" + fila[0] + " | Código de Evento=" + fila[1]);
                }
            } else {
                System.out.println("No se encontraron registros de desconexión.");
            }
        } catch (Exception e) {
            System.err.println("Error en Disconnect Log: " + e.getMessage());
        }
        return result;
    }
}
